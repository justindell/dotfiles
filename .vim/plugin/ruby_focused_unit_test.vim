...
class RubyFocusedUnitTest
  ...
  def run_unit_test
    ...
    write_output_to_buffer("jruby #{current_file} -n #{method_name}") if method_name
  end
  ...
  def run_context
    ...
    if method_name
      if spec_file?
        ...
        write_output_to_buffer("jruby #{current_file} -n #{method_name}")
      end
    end
  end
  ...
  def run_all
    if spec_file?
      ...
      write_output_to_buffer("jruby #{current_file}")
    end
  end
